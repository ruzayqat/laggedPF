...
import h5py
from tools import (mat_mul, fwd_slash, mvnrnd, nearestpd)
    ...
    for step in tqdm(range(params["T"])):
        ...
        if (step + 1) % params["t_freq"] == 0:
            ...
            if params["C_is_eye"]:
                ...
                comp["noise"] = mvnrnd(np.zeros((params["dimo"])), comp["id_o"], coln=params["M"])
                ...
                temp = mat_mul(comp["diff"].T, temp)
                comp["kappa"] = mat_mul(comp["diff"], temp) / params["M"]
                comp["noise"] = mvnrnd(np.zeros((params["dimo"])), comp["id_o"],
                                       coln=params["M"])
                if params["Y_CoefMatrix_is_eye"]:
                    ...
                    temp = data.reshape(-1, 1) - mat_mul(params["C"], x_f) \
                           - mat_mul(noised_obs_mat, comp["noise"])
            x_a = x_f + mat_mul(comp["kappa"], temp)
        else:
            x_a = x_f
        cov_array = nearestpd(cov_array)
        _dump(params, step, pred_mean, cov_array)
    return pred_mean, cov_array
...
def _dump(params, step, pred_mean, cov_array):
    # print("Dumping predictor stats to %s..." %params["predictor_file"])
    ...
    with h5py.File(params["predictor_file"], "a") as fout:
        ...
        grp.create_dataset(name="cov_array", data=cov_array)
