...
class ShallowWaterSolver():
    ...
    def __init__(self, dim, step=1, gravity=9.81, cfl=0.5):
        ...
        self.solution = np.zeros((3, dim, dim))
        self.fluxes = {"lam_u": np.zeros((dim, dim)),
                       "lam_v": np.zeros((dim, dim)),
                       "x": np.zeros((3, dim, dim)),
                       "y": np.zeros((3, dim, dim))}
        self.gravity = gravity
        self.shift = {"m1": np.roll(np.arange(dim), -1),
                      "p1":np.roll(np.arange(dim), 1)}
    ...
    def init(self, sol_prev):
        ...
        for i in range(3):
            self.solution[i, :, :] = sol_prev[i*dim2:(i+1)*dim2].reshape(dim, dim)
    ...
    def solve(self):
        ...
        self._advance_sol()
        self._apply_bc()
        return self.solution.reshape(-1), self.step["t"]
    ...
    def dump(self, prefix, ite):
        ...
        with h5py.File(filename, "w") as fout:
            for i, key in enumerate(keys):
                fout.create_dataset(name=key, data=self.solution[i, :, :])
